...
void BUTTON_INIT(void)
{
	...
	GPIOC->MODER |= 1UL<<(2*3);						//ONLY set  GPIOC (PC3)
	GPIOC->MODER &= ~(2UL<<(2*3));				//ONLY clear  GPIOC (PC3)
	GPIOC->OTYPER &= ~(1UL<<3); 					//Configure PC3 as Push-Pull
	
	GPIOA->MODER |= 1UL<<(2*LED_Toggling);						//ONLY set  GPIOA (PA7)
	GPIOA->MODER &= ~(2UL<<(2*LED_Toggling));				//ONLY clear  GPIOA (PA7)
	...
	GPIOG->MODER 	&= ~(1UL<<(2*buttonA)); 				//ONLY CLEAR GPIOG (PG0) TO SET AS INPUT
	GPIOG->MODER 	&= ~(1UL<<(2*buttonB)); 				//ONLY CLEAR GPIOG (PG1) TO SET AS INPUT
	GPIOG->MODER 	&= ~(1UL<<(2*buttonC)); 				//ONLY CLEAR GPIOG (PG2) TO SET AS INPUT
	GPIOG->MODER 	&= ~(1UL<<(2*buttonD)); 				//ONLY CLEAR GPIOG (PG3) TO SET AS INPUT
	
	
	GPIOG->MODER &= ~(2UL<<(2*buttonA)); 				//ONLY CLEAR GPIOG (PG0) TO SET AS INPUT
	GPIOG->MODER &= ~(2UL<<(2*buttonB)); 				//ONLY CLEAR GPIOG (PG1) TO SET AS INPUT
	GPIOG->MODER &= ~(2UL<<(2*buttonC)); 				//ONLY CLEAR GPIOG (PG2) TO SET AS INPUT
	GPIOG->MODER &= ~(2UL<<(2*buttonD)); 				//ONLY CLEAR GPIOG (PG3) TO SET AS INPUT
	...
	//SET ALL TO LOW SPEED***
	GPIOC->OSPEEDR &= ~(1UL<<(2*blueButton));			//ONLY CLEAR GPIOC (PC13) TO SET TO LOW SPEED
	
	GPIOG->OSPEEDR |= 1UL<<(2*buttonA); 					//ONLY SET GPIOG (PG0) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR |= 1UL<<(2*buttonB); 					//ONLY SET GPIOG (PG1) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR |= 1UL<<(2*buttonC); 					//ONLY SET GPIOG (PG2) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR |= 1UL<<(2*buttonD); 					//ONLY SET GPIOG (PG3) TO SET TO HIGH SPEED
	
	GPIOC->OSPEEDR &= ~(2UL<<(2*blueButton));			//ONLY CLEAR GPIOC (PC13) TO SET TO LOW SPEED
	
	GPIOG->OSPEEDR &= ~(2UL<<(2*buttonA));				//ONLY CLEAR GPIOG (PG0) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR &= ~(2UL<<(2*buttonB));				//ONLY CLEAR GPIOG (PG1) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR &= ~(2UL<<(2*buttonC));				//ONLY CLEAR GPIOG (PG2) TO SET TO HIGH SPEED
	GPIOG->OSPEEDR &= ~(2UL<<(2*buttonD));				//ONLY CLEAR GPIOG (PG3) TO SET TO HIGH SPEED


	
	GPIOC->PUPDR &= ~(1UL<<(2*blueButton)); 						//ONLY SETCLEAR GPIOC (PC13) TO SET AS PULL DOWN
	
	GPIOG->PUPDR |= 1UL<<(2*buttonA);						//ONLY SETCLEAR GPIOG (PG0) TO SET TO PULL UP
	GPIOG->PUPDR |= 1UL<<(2*buttonB);						//ONLY SETCLEAR GPIOG (PG1) TO SET TO PULL UP
	GPIOG->PUPDR &= ~(1UL<<(2*buttonC));						//ONLY SETCLEAR GPIOG (PG2) TO SET TO PULL DOWN
	GPIOG->PUPDR &= ~(1UL<<(2*buttonD));						//ONLY SETCLEAR GPIOG (PG3) TO SET TO PULL DOWN
	...
	GPIOG->PUPDR |= (2UL<<(2*buttonC));					//ONLY CLEAR GPIOG (PG2) TO SET AS PULL DOWN
	GPIOG->PUPDR |= (2UL<<(2*buttonD));					//ONLY CLEAR GPIOG (PG3) TO SET AS PULL DOWN
}
...
void INIT_TIM12(void)
{
	...
	TIM12->ARR = 10000 - 1;  // Auto-reload: 10kHz / 1000 = 10Hz DAC update rate
...
void TIM8_BRK_TIM12_IRQHandler(void)
{
    if ((TIM12->SR & TIM_SR_UIF) != 0) {  // Check if update event occurred
        ...
        GPIOA->ODR ^= 1<<LED_Toggling; //TOGGLE PA7 TO TURN ON AND OFF RED AND INFRARED LED SIMULTANEOUSLY
		}
}
